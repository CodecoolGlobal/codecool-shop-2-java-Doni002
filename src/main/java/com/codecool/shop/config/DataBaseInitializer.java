...
/*        try {
        ...
        //Motorcycles
        productDataStore.add(new Product("ARCH NAZARE", new BigDecimal("138000"), "USD", "The roar of one of these beauties gives all those corpos stuck in their sluggish, armor-heavy limos something to mull over in traffic. What if, instead of those endless quarterly report meetings or - barf - corporate responsibility retreats, they just hopped on one of these bad boys, got that motor runnin', and drove off into the sunset? Sigh… Because the ARCH Nazare isn't just a bike, it's a way of life.", motorCycle, ARCH));
